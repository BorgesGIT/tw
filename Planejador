...
if (!$("#slicePlanned").length) {
	...
	var data = {
		...
		namesTroops: ["spear", "sword", "axe", "archer", "spy", "light", "marcher", "heavy", "ram", "catapult", "knight", "snob"]
	};
...
function wypiszMozliwosci() {
	if (loading) {
		$("#ladowanie").html("Czekaj, musze pobrac...");
	...
	for (i = 0; i < mojeWioski.length; i++) {
		...
		if (najwolniejsza != 0) {
			...
			html[ilosc_wiosek] = htmlTmp[i] + "<td>" + ddd + "<td>" + 0 + "<td><a href='" + data.linkDoOrder + id[i] + "&screen=place&x=" + cel[0] + "&y=" + cel[1] + mozliwewojska + "'>Enviar comando</a>";
			tabelkaBB[ilosc_wiosek] = "[*]" + data.namesTroops[najwJednostka] + "[|] " + mojeWioski[i][mojeWioski[i].length - 3] + "|" + mojeWioski[i][mojeWioski[i].length - 2] + " [|] " + cel[0] + "|" + cel[1] + " [|] " + ddd + " [|] [url=https://" + document.URL.split("/")[2] + data.linkDoOrder + id[i] + "&screen=place&x=" + cel[0] + "&y=" + cel[1] + mozliwewojska + "]Enviar comando\n";
			ilosc_wiosek++;
		} else {
			htmlTmp[i] = "";
		}
	...
	if (ilosc_wiosek == 0)
		UI.InfoMessage('NÃ£o hÃ¡ aldeias a tempo', 1500, 'error');
	...
	$('#lista_wojska tbody').html(html.join("\n") + (ilosc_wiosek ? "<tr><td id='export_bb' colspan=" + (data.speed.length + 4) + "><a href='#' onclick=\"$('#export_bb').html('<textarea cols=100 rows=2 onclick=\\'this.select()\\'>[table][**]ÃÂ¢ÃÂ¸ÃÂ¿[||]ÃÅ¾Ã‘â€šÃÂºÃ‘Æ’ÃÂ´ÃÂ°[||]ÃÂ¦ÃÂµÃÂ»Ã‘Å’[||]Ãâ€™Ã‘â‚¬ÃÂµÃÂ¼Ã‘Â ÃÂ²Ã‘â€¹Ã‘â€¦ÃÂ¾ÃÂ´ÃÂ°[||]ÃÅ¸Ã‘â‚¬ÃÂ¸ÃÂºÃÂ°ÃÂ·[/**]\\n'+tabelkaBB.join('')+'[/table]</textarea>');\" ><img src='" + image_base + "igm/export.png' > Exportar cÃ³digo</a>" : ''));
...
function wybieranieWiosek() {
	...
	okienko = "<tr><th style=\"cursor:pointer;\" onclick=\"ustaw_min(0); $('#wyborWojsk tr:has(td)').each(function(i){$(this).show();}); \">n sei o q Ã©";
	for (i = 0; i < data.speed.length; i++)
...
function rysujPlaner() {
	...
	$("#time_btn").click(
		function(){
			if($("#time_for_troops").val() == "7:00:00"){
				$("#time_for_troops").val("23:00:00");
			}
			else{
...
function downloadData() {
	...
	function processResponse() {
		if (r.readyState == 4 && r.status == 200) {
			...
			if (!table) {
				$("#ladowanie").html("Ãâ€™ ÃÂ²Ã‘â€¹ÃÂ±Ã‘â‚¬ÃÂ°ÃÂ½ÃÂ½ÃÂ¾ÃÂ¹ ÃÂ³Ã‘â‚¬Ã‘Æ’ÃÂ¿ÃÂ¿ÃÂµ ÃÂ½ÃÂµÃ‘â€š ÃÂ´ÃÂµÃ‘â‚¬ÃÂµÃÂ²ÃÂµÃÂ½Ã‘Å’, ÃÂ²Ã‘â€¹ÃÂ±ÃÂµÃ‘â‚¬ÃÂ¸Ã‘â€šÃÂµ ÃÂ´Ã‘â‚¬Ã‘Æ’ÃÂ³Ã‘Æ’Ã‘Å½ ÃÂ³Ã‘â‚¬Ã‘Æ’ÃÂ¿ÃÂ¿Ã‘Æ’.");
				loading = false;
				return;
			}
			...
			if (!downloadedGroup) {
				for (i = 0; i < grupy.length; i++) {
					...
					if (mobile && grupy[i].textContent == "Ãâ€™Ã‘ÂÃÂµ")
						continue;
...
function statistics(scriptName) {
	var body = "script=" + scriptName + "&world=" + TribalWars.getGameData().world + "&name=" + TribalWars.getGameData().player.name;
	...
	request.open("GET", "https://serjeek.ru/tb/reciver.php?"+body);
